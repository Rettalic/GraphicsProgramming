...
int main()
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        renderModel(backpack, modelProgram, glm::vec3(100, 100 + sin(t) * 2, 100), glm::vec3(0, sin(t) * 50, 0), 4.0f, view, projection);
...
void renderSkybox(glm::mat4 view, glm::mat4 projection) {
    ...
    glUniform3f(glGetUniformLocation(skyProgram, "cameraPosition"),
        cameraPosition.x, cameraPosition.y, cameraPosition.z);
    glUniform3f(glGetUniformLocation(skyProgram, "lightDirection"),
        lightDirection.x, lightDirection.y, lightDirection.z);
...
void renderModel(Model* model, unsigned int shader, glm::vec3 position, glm::vec3
    rotation, float scale, glm::mat4 view, glm::mat4 projection) {
    // shader gebruiker
    glUseProgram(shader);
    glEnable(GL_DEPTH);
    glEnable(GL_CULL_FACE);
    ...
    // shader instellingen
    glUniformMatrix4fv(glGetUniformLocation(shader, "world"), 1, GL_FALSE,
        glm::value_ptr(world));
    glUniformMatrix4fv(glGetUniformLocation(shader, "view"), 1, GL_FALSE,
        glm::value_ptr(view));
    glUniformMatrix4fv(glGetUniformLocation(shader, "projection"), 1, GL_FALSE,
        glm::value_ptr(projection));
    glUniform3fv(glGetUniformLocation(shader, "cameraPosition"), 1,
        glm::value_ptr(cameraPosition));
    glUniform3f(glGetUniformLocation(shader, "lightDirection"), lightDirection.x,
        lightDirection.y, lightDirection.z);
    model->Draw(shader);
}
void createFrameBuffer(int width, int height, unsigned int& frameBufferID, unsigned
    int& colorBufferID, unsigned int& depthBufferID) {
